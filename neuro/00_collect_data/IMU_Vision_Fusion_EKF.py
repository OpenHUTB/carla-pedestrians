...
class EKF_VIO:
    def __init__(self, init_pose, init_vel, dt=0.05):
        ...
        # 过程噪声（大幅调大→显著降低IMU信任, VO可有效修正IMU累积漂移）
        self.Q = np.diag([0.50, 0.50, 0.01, 1.00, 1.00, 0.50, 0.03, 0.03, 0.03])
        # VO观测噪声（持续微调→VO短期精度更多注入, RPE继续向纯VO收敛）
        self.R = np.diag([0.005, 0.005, 0.0015, 0.0015, 0.0015, 0.0015])
