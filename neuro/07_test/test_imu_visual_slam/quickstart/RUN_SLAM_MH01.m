%% 运行EuRoC MH_01_easy SLAM测试
%  支持从原始EuRoC数据集读取IMU数据
%
...
VT_MATCH_THRESHOLD_OVERRIDE = 0.025;  
% Experience Map 
global DELTA_EXP_GC_HDC_THRESHOLD_OVERRIDE;
DELTA_EXP_GC_HDC_THRESHOLD_OVERRIDE = 6;  

% Map Relaxation 
global EXP_LOOPS_OVERRIDE;
global EXP_CORRECTION_OVERRIDE;
EXP_LOOPS_OVERRIDE = 25;       
EXP_CORRECTION_OVERRIDE = 0.75; 

% IMU融合
global IMU_YAW_WEIGHT_OVERRIDE;
global IMU_TRANS_WEIGHT_OVERRIDE;
global IMU_HEIGHT_WEIGHT_OVERRIDE;
IMU_YAW_WEIGHT_OVERRIDE = 0.95;   
IMU_TRANS_WEIGHT_OVERRIDE = 0.80; 
IMU_HEIGHT_WEIGHT_OVERRIDE = 0.80;

global GC_VT_INJECT_ENERGY_OVERRIDE;
GC_VT_INJECT_ENERGY_OVERRIDE = 0.2; 

fprintf('\n╔════════════════════════════════════════════════╗\n');
fprintf('║   EuRoC MH_01 SLAM测试                       ║\n');
fprintf('║   惯视融合 + 类脑SLAM                     ║\n');
fprintf('╚════════════════════════════════════════════════╝\n\n');
...
EUROC_DATA_PATH = fullfile(neuro_root, 'data', '02_EuRoc_Dataset', 'MH_01_easy');

% 方法3：手动指定（如果上面的路径不对）
if ~exist(EUROC_DATA_PATH, 'dir')
    fprintf('⚠️  数据路径不存在: %s\n\n', EUROC_DATA_PATH);
    fprintf('请设置正确的EUROC数据路径，例如：\n');
    fprintf('  EUROC_DATA_PATH = ''/home/user/EuRoC/MH_01_easy'';\n\n');
    
    % 提示用户输入
    user_path = input('请输入EuRoC MH_01_easy数据路径（留空取消）: ', 's');
...
%% 运行测试
fprintf('启动EuRoC SLAM测试...\n\n');

if has_imu
    fprintf('✓ 将使用真实IMU-视觉融合\n');
    fprintf('  预期输出:\n');
    fprintf('  - 纯视觉轨迹\n');
    fprintf('  - IMU-aided轨\n');
    fprintf('  - 生物启发SLAM轨迹（完整系统）\n\n');
else
    fprintf('⚠️  纯视觉模式\n');
    fprintf('  如需测试IMU融合，请确保数据集包含IMU数据\n\n');
end
...
% 提示下一步
fprintf('下一步:\n');
fprintf('  1. 查看对比图: open %s/imu_visual_slam_comparison.png\n', result_path);
fprintf('  2. 加载数据分析: load(''%s/euroc_trajectories.mat'')\n', result_path);
fprintf('  3. 测试其他序列: 修改数据路径为MH_02, MH_03等\n\n');
