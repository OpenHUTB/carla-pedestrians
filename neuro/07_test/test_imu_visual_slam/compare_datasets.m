%% 数据集对比分析：Town01 vs Town10 vs EuRoC MH_01
%  对比不同数据集上的SLAM性能
%  生成详细的可视化对比图表
...
fprintf('\n╔════════════════════════════════════════════════════════════╗\n');
fprintf('║  数据集性能对比分析                                        ║\n');
fprintf('║  Town01 (虚拟) vs Town10 (虚拟) vs EuRoC MH_01 (真实)      ║\n');
fprintf('╚════════════════════════════════════════════════════════════╝\n\n');
...
% Town01
town01_path = '../../data/01_NeuroSLAM_Datasets/Town01Data_IMU_Fusion/slam_results';
...
% Town10
town10_path = '../../data/01_NeuroSLAM_Datasets/Town10Data_IMU_Fusion/slam_results';
...
% EuRoC MH_01
euroc_path = '/home/dream/neuro_111111/datasets/euroc_converted/MH_01_easy/slam_results';
if exist(fullfile(euroc_path, 'euroc_trajectories.mat'), 'file')
    euroc = load(fullfile(euroc_path, 'euroc_trajectories.mat'));
...
if has_euroc
    datasets{end+1} = 'EuRoC';
    metrics.EuRoC = calculate_metrics_euroc(euroc);
    fprintf('  EuRoC:  轨迹长度 %.1fm\n', metrics.EuRoC.traj_length);
end
...
% 创建输出目录
output_dir = '/home/dream/neuro_111111/datasets/comparison_results';
...
for i = 1:length(datasets)
    ...
    % 鲁棒性（终点误差越小越好）
    final_errors = [metrics.(datasets{:}).fusion_final_error];
    radar_data(i, 5) = (1 - (metrics.(ds).fusion_final_error - min(final_errors)) / (max(final_errors) - min(final_errors) + 0.01)) * 100;
    % 综合性能（前5项平均）
...
for i = 1:length(datasets)
    ...
    polarplot(theta, values, '-o', 'LineWidth', 2.5, ...
        'Color', colors(i, :), 'MarkerSize', 8, ...
        'MarkerFaceColor', colors(i, :), 'DisplayName', datasets{i});
    % 填充半透明区域
    polarplot(theta, values, 'LineWidth', 0.5, 'Color', [colors(i, :), 0.1]);
    fill(theta, values, colors(i, :), 'FaceAlpha', 0.15, 'EdgeColor', 'none');
end
legend('Location', 'northeast', 'FontSize', 11);
title('多维度性能雷达图', 'FontSize', 14, 'FontWeight', 'bold');
rlim([0, 100]);
thetaticks(0:360/num_cats:360-360/num_cats);
...
function m = calculate_metrics(data)
    ...
    % 对齐轨迹
    [fusion_aligned, gt_aligned] = align_trajectories(data.fusion_data.pos, data.gt_data.pos, 'simple');
    [odo_aligned, ~] = align_trajectories(data.odo_trajectory, data.gt_data.pos, 'simple');
    [exp_aligned, ~] = align_trajectories(data.exp_trajectory, data.gt_data.pos, 'simple');
