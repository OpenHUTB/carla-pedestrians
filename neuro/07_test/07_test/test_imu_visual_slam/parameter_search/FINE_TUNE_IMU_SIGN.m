%% 精细调参：测试IMU符号和小范围alpha_yaw
%  
%  问题：alpha_yaw > 0 时ATE反而变差
%  可能原因：
%    1. IMU yaw符号错误（需要取反）
%    2. alpha_yaw太大
%  
%  解决方案：
%    1. 测试 IMU_YAW_SIGN = -1
%    2. 测试更小的 alpha_yaw (0.01 ~ 0.05)

clear; clc; close all;

fprintf('╔══════════════════════════════════════════════════════════════╗\n');
fprintf('║     精细调参：测试IMU符号和小范围alpha_yaw                   ║\n');
fprintf('╚══════════════════════════════════════════════════════════════╝\n\n');
...
% 测试小范围alpha_yaw
alpha_yaw_values = [0.0, 0.01, 0.02, 0.03, 0.05, 0.08, 0.10];
...
%% 运行Baseline
fprintf('\n[2/4] 运行Baseline...\n');
[baseline_traj, ~] = run_slam_get_trajectory_v2(data_path, img_files, [], num_frames, rootDir, 'baseline', 1);
...
%% 网格搜索
fprintf('\n[3/4] 测试不同IMU符号和alpha_yaw...\n');

all_results = [];

for imu_sign = imu_signs
    fprintf('\n=== IMU符号 = %d ===\n', imu_sign);
    
    for alpha_yaw = alpha_yaw_values
        fprintf('  测试 alpha_yaw=%.2f... ', alpha_yaw);
        ...
        % 运行Ours SLAM
        [ours_traj, ours_vt] = run_slam_get_trajectory_v2(data_path, img_files, imu_data, num_frames, rootDir, 'ours', imu_sign);
...
fprintf('╠════════════════════════════════════════════════════════════════╣\n');
fprintf('║  IMU符号 │ alpha_yaw │   ATE (m)  │  改进 (%%)                  ║\n');
fprintf('╠════════════════════════════════════════════════════════════════╣\n');
...
for i = 1:length(all_results)
    ...
    fprintf('║    %s    │   %.2f    │  %7.2f   │  %+7.2f                  ║\n', ...
        sign_str, r.alpha_yaw, r.ate, r.improvement);
...
fprintf('║  最优: IMU符号=%s, alpha_yaw=%.2f, ATE=%.2fm, 改进=%+.2f%%    ║\n', ...
    sign_str, best.alpha_yaw, best.ate, best.improvement);
fprintf('╚════════════════════════════════════════════════════════════════╝\n');
...
save_path = fullfile(data_path, 'fine_tune_imu_results.mat');
...
function [traj, vt_count] = run_slam_get_trajectory_v2(data_path, img_files, imu_data, num_frames, rootDir, mode, imu_sign)
% 运行SLAM并返回轨迹
