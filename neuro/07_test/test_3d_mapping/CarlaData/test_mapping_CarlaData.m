...
% vo initial
visual_odo_initial( ...
    'ODO_IMG_TRANS_Y_RANGE', 31:90, ...      % 里程计图像平移范围（x, y）
    'ODO_IMG_TRANS_X_RANGE', 16:145, ...
    'ODO_IMG_HEIGHT_V_Y_RANGE', 11:110, ...  % 里程计图像高度（俯仰）垂直范围(x, y)
    'ODO_IMG_HEIGHT_V_X_RANGE', 11:150, ...
    'ODO_IMG_YAW_ROT_Y_RANGE', 31:90, ...    % 里程计图像偏航（旋转）范围(x, y)
    'ODO_IMG_YAW_ROT_X_RANGE', 16:145, ...
    'ODO_IMG_TRANS_RESIZE_RANGE', [60, 130], ...    % 定义里程计调整大小后的图像尺寸
    'ODO_IMG_YAW_ROT_RESIZE_RANGE', [60, 130], ...
    'ODO_IMG_HEIGHT_V_RESIZE_RANGE', [100, 140], ...
    'ODO_TRANS_V_SCALE', 30, ...        % 平移速度的尺寸
    'ODO_YAW_ROT_V_SCALE', 1, ...       % 旋转速度的尺寸
    'ODO_HEIGHT_V_SCALE', 20, ...       % 俯仰速度的尺寸
    'MAX_TRANS_V_THRESHOLD', 0.5, ...       % 最大平移速度的最大阈值
    'MAX_YAW_ROT_V_THRESHOLD', 2.5, ...     % 最大偏航速度的最大阈值
    'MAX_HEIGHT_V_THRESHOLD', 0.45, ...     % 最大俯仰速度的最大阈值
    'ODO_SHIFT_MATCH_HORI', 26, ...     % 水平方向的视觉里程计位移匹配变量
    'ODO_SHIFT_MATCH_VERT', 20, ...     % 垂直方向的视觉里程计位移匹配变量
    'FOV_HORI_DEGREE', 81.5, ...            % 水平视野的角度
    'FOV_VERT_DEGREE', 50, ...              % 垂直视野的角度
...
% 偏航、俯仰、头部朝向细胞 yaw_pitch_hdc 初始化
