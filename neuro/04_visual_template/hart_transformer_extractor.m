...
%   normImg - 归一化特征图 [H×W]
    ...
    % Ventral Stream（特征）
    local_std = stdfilt(attended_img, ones(5,5));
    ventral_enhanced = adapthisteq(attended_img, 'ClipLimit', 0.02, 'NumTiles', [4 4]);
    ...
    % Self-Attention效果：用全局信息调制局部特征（Plan B最优配置）
    % Head 1: Dorsal特征增强
    ...
    %% ========== 步骤7: 特征融合（Plan B最优配置） ==========
    % 融合所有层次的特征（加权平均）
    fused_features = 0.20 * dorsal_features + ...      % 位置信息
                     0.30 * lstm_h + ...               % 时序记忆
                     0.30 * transformer_features + ... % 全局上下文
                     0.20 * v1_output;                 % 基础特征
