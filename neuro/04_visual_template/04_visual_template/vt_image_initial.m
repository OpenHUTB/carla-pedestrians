function vt_image_initial(imgType, varargin)
...
%     along with this program.  If not, see <http://www.gnu.org/licenses/>.
    ...
    for i=1:(nargin-1)
        if ischar(varargin{i})
            switch varargin{i}
                ...
                case 'VT_PANORAMIC', VT_PANORAMIC = varargin{i+1};
                case 'VT_STEP', VT_STEP = varargin{i+1};
    ...
    [curYawTheta, curPitchTheta] = get_hdc_initial_value();
    ...
    VT(1).numExp = 1;
    VT(1).EXPERIENCES(1).id = 1;
        
    VT(1).template = zeros(VT_IMG_RESIZE_Y_RANGE, VT_IMG_RESIZE_X_RANGE);
