...
%   dataset_name - 数据集名称
    ...
    radar_labels = {'Accuracy', 'Precision', 'Smoothness', 'Drift Control', 'Consistency'};
    n_dims = length(radar_labels);
    
    % 计算归一化分数 (反转误差指标，使得越大越好)
    max_rmse = max([metrics.rmse]);
    max_drift = max([metrics.drift_rate]);
    max_rpe = max([metrics.rpe]);
    max_smooth = max([metrics.smoothness]) + 0.001;
    max_std = max([metrics.std_error]);
    
    radar_data = zeros(n_methods, n_dims);
    for i = 1:n_methods
        radar_data(i, 1) = 1 - metrics(i).rmse / max_rmse;  % Accuracy
        radar_data(i, 2) = 1 - metrics(i).rpe / max_rpe;    % Precision
        radar_data(i, 3) = 1 - metrics(i).smoothness / max_smooth;  % Smoothness
        radar_data(i, 4) = 1 - metrics(i).drift_rate / max_drift;   % Drift Control
        radar_data(i, 5) = 1 - metrics(i).std_error / max_std;      % Consistency
    end
    ...
    for i = 1:n_methods
        ...
        % 计算核密度估计
        [f, xi] = ksdensity(errors, 'NumPoints', 100);
