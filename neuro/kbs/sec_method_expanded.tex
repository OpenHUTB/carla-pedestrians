...
NeuroLocMap integrates a dual-stream visual front-end, a ves\-tib\-u\-lar (IMU) front-end, and hippocampal--entorhinal spatial representations to estimate 4-DoF pose $(x,y,z,\psi)$ and build maps in challenging 3D environments. As shown in Figure~\ref{fig:system_overview}, the visual front-end extracts appearance and motion cues along ventral (V1$\rightarrow$V2$\rightarrow$V4$\rightarrow$IT) and dorsal (V1$\rightarrow$MT/MST) path\-ways, while the ves\-tib\-u\-lar front-end provides high-frequency self-motion from gyroscopes and accelerometers. Com\-ple\-men\-tary filtering fuses these modalities in the frequency domain. Spatial representation uses 3D grid cells for $(x,y,z)$ and multilayer head-direction cells for $\psi$ with path integration and visual anchoring to limit drift. A hybrid experience map couples visual templates with spatial-cell snapshots to support loop closure and pose-graph relaxation, yielding globally consistent topology with local metric accuracy.
