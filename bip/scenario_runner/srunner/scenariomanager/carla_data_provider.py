...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    @staticmethod
    def get_next_junction_id(actor):
        """
        returns the next junction id of the actor
        """

        location = actor.get_transform().location

        waypoint = CarlaDataProvider.get_map().get_waypoint(location, lane_type=carla.LaneType.Driving)
        while not waypoint.is_junction:
            waypoint = waypoint.next(2.0)[0]

        return waypoint.get_junction().id
    ...
    @staticmethod
    def get_actor_blueprints(model, disable_two_wheels=False, disable_giant=False):
        """
        Function to get the blueprints of a specific class of actors
        """
        try:
            blueprints = CarlaDataProvider._blueprint_library.filter(model)
            # we don't spawn vehicles that with not suitable size in Town2, and we disable two wheels vehicles
            if 'vehicle' in model:
            ...
            print("WARNING: Actor model {} not available.".format(model))
    ...
    @staticmethod
    def request_new_batch_actors(models_name_list, models_amount_list, spawn_points, autopilot=False,
                                 random_location=False, rolename='scenario', disable_giant=False, disable_two_wheels=False):
        """
        ...
        CarlaDataProvider.generate_spawn_points()
        if models_amount_list[0] > len(CarlaDataProvider._spawn_points):
            print("No enough spawn points to be used for {}. Could only find {} spawn points".format(models_name_list[0], len(CarlaDataProvider._spawn_points)))
            models_amount_list[0] = len(CarlaDataProvider._spawn_points)

        blueprints = CarlaDataProvider.get_actor_blueprints(models_name_list[0], disable_giant=disable_giant, disable_two_wheels=disable_two_wheels)
        blueprintWalkers = CarlaDataProvider.get_actor_blueprints(models_name_list[1])
        blueprintWalkers_controller = CarlaDataProvider.get_actor_blueprints('controller.ai.walker')

        # --------------
        # Spawn vehicles
        # --------------
        batch = []
        for i in range(models_amount_list[0]):
            ...
            if blueprint.has_attribute('driver_id'):
                driver_id = CarlaDataProvider._rng.choice(blueprint.get_attribute('driver_id').recommended_values)
            ...
            if random_location:
                if CarlaDataProvider._spawn_index >= len(CarlaDataProvider._spawn_points):
                    print("No more spawn points to use. Spawned {} actors out of {}".format(i + 1, models_amount_list[0]))
                    break
                else:
        ...
        vehicles_list=[]
        for response in CarlaDataProvider._client.apply_batch_sync(batch, True):
            if not response.error:
                vehicles_list.append(response.actor_id)

        carla_actors = CarlaDataProvider._world.get_actors(vehicles_list)
        for actor in carla_actors:
            actors.append(actor)
        ...
        for spawn_point in walkers_spawn_points:
            walker_bp = CarlaDataProvider._rng.choice(blueprintWalkers)
            # set as not invincible
            if walker_bp.has_attribute('is_invincible'):
                walker_bp.set_attribute('is_invincible', 'false')
            # set the max speed
            if walker_bp.has_attribute('speed'):
                if (CarlaDataProvider._rng.random() > 0.0):
                    # walking
        ...
        results = CarlaDataProvider._client.apply_batch_sync(batch, True)
        walkers_speed_tmp = []
        walkers_list = []
        for i in range(len(results)):
            if not results[i].error:
                walkers_list.append({"id": results[i].actor_id})
                walkers_speed_tmp.append(walkers_speed[i])
        walkers_speed = walkers_speed_tmp

        # 3. we spawn the walker controller
        batch = []
        for i in range(len(walkers_list)):
            batch.append(SpawnActor(blueprintWalkers_controller[0], carla.Transform(), walkers_list[i]["id"]))
        results = CarlaDataProvider._client.apply_batch_sync(batch, True)
        for i in range(len(results)):
            if not results[i].error:
        ...
        all_walker_actors = CarlaDataProvider._world.get_actors(all_walkers_id)
        ...
        for i in range(0, len(all_walkers_id), 2):
            # start walker
            all_walker_actors[i].start()
            # set walk to random point
            all_walker_actors[i].go_to_location(CarlaDataProvider._world.get_random_location_from_navigation())
            # max speed
            all_walker_actors[i].set_max_speed(float(walkers_speed[int(i / 2)]))
            actors.append(all_walker_actors[i+1])
    ...
    def cleanup():
        ...
        CarlaDataProvider._traffic_manager_port = 8000
